...
pub fn ensure_not_root() -> Result<()> {
    ...
    let output = Command::new("id").arg("-u").output().context("Failed to run id")?;
    let uid = String::from_utf8_lossy(&output.stdout).trim().parse::<u32>().context("Failed to parse uid")?;
    
    if uid == 0 {
        bail!("Please do NOT run slate as root. It uses sudo internally where needed.\n\
...
pub fn trace_to_physical_partition(device_path: &str) -> Result<String> {
    ...
    if !sys_path.exists() {
        ...
        bail!("Device {} (resolved: {}) not found in sysfs at {}", device_path, real_path.display(), sys_path.display());
    }
