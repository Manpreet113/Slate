...
Singleton {
    ...
    readonly property string configDir: (Quickshell.env("XDG_CONFIG_HOME") || (Quickshell.env("HOME") + "/.config")) + "/slate"
    ...
    function savePreset(presetName: string, configFiles: var) {
        ...
        for (const configFile of configFiles) {
            ...
            // The source is configDir (~/.config/Elysium), NOT configDir/config
            // But wait, the configDir property is defined as ~/.config/Elysium below?
            // Let's check the property definition.
            // property string configDir: ... + "/Elysium"
            // But Config.qml says configDir is ... + "/Elysium/config"
            // We need to match Config.qml's path.
            
            // In Config.qml: property string configDir: ... + "/Elysium/config"
            // Here: readonly property string configDir: ... + "/Elysium"
            // This is a mismatch!
            
            // We should use the same path as Config.qml for reading/writing config files.
            // Let's assume the files are in .../Elysium/config based on Config.qml and ls output.
