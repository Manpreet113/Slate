...
Item {
    ...
    property string currentCategory: "slate"
    ...
    property bool isEditingElysium: false
    ...
    function openEditDialog(bind, index, isElysium) {
        root.editingIndex = index;
        root.editingBind = bind;
        root.isEditingElysium = isElysium;

        // Initialize edit form state
        if (isElysium) {
            // Elysium binds still use old format (single key)
    ...
    function saveEdit() {
        if (root.isEditingElysium) {
            // Save slate bind (still uses old format internally)
            const path = root.editingBind.path.split(".");
            // path = ["slate", "section"?, "bindName"]
            
            const adapter = Config.keybindsLoader.adapter;
            if (adapter && adapter.slate) {
                let bindObj = null;
                if (path.length === 2) {
                    // Top level: slate.bindName
                    bindObj = adapter.slate[path[1]];
                } else if (path.length === 3) {
                    // Nested: slate.system.bindName
                    bindObj = adapter.slate[path[1]][path[2]];
                }
    ...
    // Get slate binds as a flat list
    function getElysiumBinds() {
        const adapter = Config.keybindsLoader.adapter;
        if (!adapter || !adapter.slate)
            return [];

        const binds = [];
        const slate = adapter.slate;

        // Core Elysium binds (Launcher, Dashboard, etc.)
            ...
            for (const key of systemKeys) {
                if (slate.system[key]) {
                    binds.push({
                        category: "System",
                        name: key.charAt(0).toUpperCase() + key.slice(1),
                        path: "slate.system." + key,
                        bind: slate.system[key]
                    });
                }
            }
        }
    ...
    Flickable {
        ...
        ColumnLayout {
            ...
            Repeater {
                id: slateRepeater
                model: root.currentCategory === "slate" ? root.getElysiumBinds() : []
            ...
            Text {
                ...
                Layout.topMargin: 20
                visible: (root.currentCategory === "slate" && slateRepeater.count === 0) || (root.currentCategory === "custom" && customRepeater.count === 0)
                text: root.currentCategory === "slate" ? "No Elysium binds configured" : "No custom binds configured"
                font.family: Config.theme.font
                font.pixelSize: Styling.fontSize(0)
            ...
            ColumnLayout {
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        StyledRect {
                            id: deleteButton
                            visible: !root.isEditingElysium && !root.isCreatingNew
                        ...
                        // Reset button (only for Elysium binds)
                        StyledRect {
                            id: resetButton
                            visible: root.isEditingElysium
                            ...
                            MouseArea {
                                ...
                                cursorShape: Qt.PointingHandCursor
                                onClicked: {
                                    if (root.isEditingElysium && root.editingBind) {
                                        const path = root.editingBind.path.split(".");
                                        // path = ["slate", "dashboard"|"system", "bindName"]
                                        const section = path[1];
                                        const bindName = path[2];
                                        
                                        // Use the new helper in Config.qml to get the default values
                                        const defaultBind = Config.keybindsLoader.adapter.getElysiumDefault(section, bindName);
                ...
                Item {
                    ...
                    ColumnLayout {
                        ...
                        // Bind name/info (for slate binds only)
                        Text {
                            visible: root.isEditingElysium && root.editingBind !== null
                        ...
                        ColumnLayout {
                            ...
                            RowLayout {
                                ...
                                StyledRect {
                                    id: removeKeyBtn
                                    visible: root.editKeys.length > 1 && !root.isEditingElysium
                        ...
                        // =====================
                        // ACTIONS SECTION (custom binds & flags for slate)
                        // =====================
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: 8
                            // visible: !root.isEditingElysium - Removed to allow editing flags for Elysium binds
                            ...
                            RowLayout {
                                ...
                                // Page indicator
                                Text {
                                    visible: root.editActions.length > 1 && !root.isEditingElysium
                            ...
                            StyledRect {
                                ...
                                radius: Styling.radius(-2)
                                opacity: root.isEditingElysium ? 0.6 : 1.0
                                ...
                                TextInput {
                                    ...
                                    verticalAlignment: Text.AlignVCenter
                                    selectByMouse: true
                                    readOnly: root.isEditingElysium
                                    onTextChanged: {
                                        if (root.editActions.length > root.currentActionPage) {
                                            const currentAction = root.editActions[root.currentActionPage];
