...
LOCKFILE="/tmp/slate_sleep_monitor.lock"
if [ -e "$LOCKFILE" ]; then
	PID=$(cat "$LOCKFILE")
...
CONFIG_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/slate/config/system.json"
...
get_cmd() {
	...
	if [ -f "$CONFIG_FILE" ]; then
		if [ "$type" == "before" ]; then
			...
			jq -r '.idle.general.after_sleep_cmd // "slate screen on"' "$CONFIG_FILE"
		fi
	else
		if [ "$type" == "before" ]; then
			echo "loginctl lock-session"
		else
			echo "slate screen on"
		fi
	fi
}
